...
def parse_args():
    ...
    parser.add_argument("--data_dir", default="./eval/data", type=str)
    ...
    parser.add_argument("--prompt_file_path", default="./eval/prompts", type=str)
    parser.add_argument("--surround_with_messages", action="store_true")
    parser.add_argument("--use_few_shot", action="store_true")
    parser.add_argument("--output_dir", default="./eval/outputs", type=str)
    parser.add_argument('--stop', type=parse_list)
    parser.add_argument("--top_p", default=1, type=float)
    parser.add_argument("--seed", default=0, type=int)
    parser.add_argument("--dtype", default='auto', type=str)
    parser.add_argument("--completions_save_dir", default='./eval/completions', type=str)
...
def get_three_prompt(prompt_type, data_name):
    file_path = os.path.join(".", "./eval/prompts", prompt_type, f"{data_name}.py")
...
def infer(args):
    ...
    if pass_at_k_list:
        average_pass_at_k = sum(pass_at_k_list) / len(pass_at_k_list)
        result_line = f"{model_name}\t{task_name}\tAcc: {correct_cnt / len(examples):.4f}\tPass@{args.k}: {average_pass_at_k:.4f}\n"
    else:
        result_line = f"{model_name}\t{task_name}\tAcc: {correct_cnt / len(examples):.4f}\tPass@1: {correct_cnt / len(examples):.4f}\n"

    result_file_path = os.path.join(args.output_dir, "results.txt")
